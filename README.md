...
|---|---|---|---|
| What it is | The game on its own page, a new word every calendar day | The game on its own page, one word per newsletter issue — **every two weeks, going live by itself** | The TMT Spark site with Spark Word built into the nav, home page, insights and Spark Learning — same fortnightly calendar |
| Backend | None — nothing to configure | Supabase (free tier) — paste two keys | Supabase (free tier) |
| Words | 100 preloaded TMT words, then repeats | 100 preloaded issues (015–114, 14 days apart); edit any of them in the dashboard | Same |
| Leaderboards | Your own results are saved on your device; other players are sample data | Real, shared, across all readers — **player accounts** (email + password or emailed link) | Real, shared, player accounts |
| Newsletter links | Any link to the page | Personal `?t=TOKEN` links that recognise each subscriber (no account needed) | Same, plus `/spark-word/015` pretty links |
...
If you're not sure, start with **Daily**. When you want real shared leaderboards and sign-in, the other two are ready: the go-live checklist for the deployed site is [`docs/GO-LIVE.md`](docs/GO-LIVE.md) and the full reference is [`docs/SETUP.md`](docs/SETUP.md).
...
## Newsletter editions (real leaderboards, every two weeks)

Both Supabase editions keep the answer, hints and scoring in Postgres: guesses are evaluated server-side, the answer never reaches the browser before the game ends, one official game per player per issue, and the `anon` key can only call the gameplay functions — it has no table access at all.

**A word every two weeks, on its own.** `supabase/spark-word-setup.sql` installs a 100-issue calendar (Issue 015 on 8 September 2026, then every 14 days). Scheduled issues go live by themselves on their date — the first player read after midnight flips the issue and archives the previous one, no cron job — and the page shows *Next word · September 22 (in 9 days)*. Editors can change any word, hint or date in the dashboard.

**Player accounts.** Guests play straight away; to be ranked they create a free account (email + password, or an emailed sign-in link) and the game they just played is attached to it. Sign in on any device and your streak, points and results follow you; "Forgot password" and sign-out are there too. Emails are never shown — leaderboards show the display name each player picks and their company. Personal newsletter links (`?t=TOKEN`, never `?email=`) still recognise subscribers with no account at all, and a subscriber who later signs up with the same email gets one merged history.

Shortest path — three files: `spark-word.html`, `spark-word-admin.html` and `supabase/spark-word-setup.sql`. Run the SQL once in the Supabase SQL editor, paste the project URL and anon key into the CONFIG block at the top of each HTML file, upload, done. For the already-deployed site the exact clicks are in **[`docs/GO-LIVE.md`](docs/GO-LIVE.md)**; the Vercel Marketplace flow, Netlify, the editor workflow, the Staffbase email recipe, the security model and the test suites are in **[`docs/SETUP.md`](docs/SETUP.md)**.
...
scripts/
  ...
  build-schedule-sql.js            Regenerates the fortnightly calendar from the same words file
...
supabase/
  spark-word-setup.sql             The whole database in one paste (schema, functions, security, word bank, dictionary, calendar)
  schedule/fortnightly.sql         The 100-issue calendar (generated by scripts/build-schedule-sql.js)
  spark-word-test-data.sql         Demo issues, players and games for a separate demo project
...
docs/
  GO-LIVE.md                       Click-by-click checklist for the deployed site (Supabase, accounts, Vercel)
...
The Supabase editions have a SQL suite (70+ assertions, including the fortnightly activation and account linking), an evaluator-parity test and three browser suites — integration, single-file, and accounts (sign-up with and without email confirmation, sign-in, password reset and magic link against a Supabase Auth look-alike); see `docs/SETUP.md` §8.
