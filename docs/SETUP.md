# Spark Word — full setup guide (Supabase editions, hosting, tests)

> This is the detailed technical guide. The repository front page is [`../README.md`](../README.md).
...
2. Run `supabase/spark-word-setup.sql` as one query — it is the migrations, the settings, word bank and dictionary seeds and `supabase/schedule/fortnightly.sql` (the 100-issue calendar, Issue 015 onwards, one word every 14 days, going live by themselves) concatenated. **Or** run the files **in this order**, each as one query:
...
### 2.7 Player accounts (sign up / sign in)

When Supabase is configured the game offers **Create account / Sign in** instead of the lightweight claim form.
It uses Supabase Auth's email provider: password sign-up (with or without email confirmation), password sign-in,
"email me a link" (magic link) and "forgot password". After any of these the client calls `sw_verified_link()`
with the session, which creates or links the `subscribers` row (names fall back to the auth user's metadata),
attaches the device's guest games, returns the subscriber token and then drops the Supabase session — identity in
the game stays token-based, exactly as for newsletter links. Configuration lives in Supabase → Authentication:
Site URL, Redirect URLs (`https://YOUR-SITE/**`), *Confirm email* on/off, and — for real players — custom SMTP,
because the built-in mailer only sends to your own project members (2 an hour). Step by step in `GO-LIVE.md`.
...
3. Sector shown to players · hint (unlocks after `hint_unlock_after` guesses — 2 with the calendar installed) · post-game explanation (pre-filled from the bank, edit for this issue)
...
bash tests/reset_local_db.sh && node tests/local-rpc-server.js &
node tests/ui.accounts.js                          # accounts: sign-up (confirm on/off), confirmation link, password sign-in,
                                                   #   wrong password, duplicate, reset link, magic link, second device
```
...
| `hint_unlock_after` | 3 (2 with the calendar) | guesses before *Need a Spark?* |
| `second_spark_after` | 5 (4 with the calendar) | guesses before the first-letter nudge (0 = off) |
| `timezone` | `UTC` | when a scheduled issue's date "arrives" (e.g. `Europe/London`) |
